...
/* Superclass/Blueprint (Base class)
 * 
 * 
 * */
...
public abstract class Problem                    // Minimization task
{
    ...
    public double[][] instanceMatrix;
	public double[][] create2DIntMatrixFromFile(String filename) throws Exception {
		instanceMatrix = new double[512][16];
		...
		while (in.hasNextLine()) {
		  String[] currentLine = in.nextLine().replaceAll(".*:", "").trim().split(",");
		  for (int i = 0; i < currentLine.length; i++) {
		   	 instanceMatrix[lineCount][i] = Double.parseDouble(currentLine[i]);    
		  }
		  	lineCount++;
		}                                 
